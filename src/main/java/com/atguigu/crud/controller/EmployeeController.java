...
public class EmployeeController {
	 ...
	 * 				会把POJO的每个属性值，request.getParameter("email");
	 * AJAX发送PUT请求引发的血案
	 * 			PUT请求，请求体中的数据request.getParameter("empName")拿不到
	 * 			Tomcat一看是PUT请求就不会封装请求体中的数据为map，只有POST形式的请求才封装请求体为map
	 * 解决方法：web.xml加上HttpPutFormContentFilter
	 * 她的作用：将请求体中的数据解析包装成一个map
	...
	public Msg getEmpsWithJson(@RequestParam(value="pn",defaultValue="1")Integer pn) {
		//这不是一个分页查询:
		//引入PageHelper分页插件
		//在查询之前需调用,传入页码，以及每页的大小
		PageHelper.startPage(pn, 5);
		//startPage后面紧跟的这个查询就是一个分页查询
		List<Employee> emps=employeeService.getAll();
		//使用pageInfo包装查询后的结果,只需要将pageInfo交给页面就行了。
		//f封装了详细的分页信息，包括了我们查询出来的数据,传入连续显示的页数
